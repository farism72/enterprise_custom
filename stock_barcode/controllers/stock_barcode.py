...
class StockBarcodeController(http.Controller):
    ...
    def _try_open_picking_type(self, barcode):
        ...
        if picking_type:
            ...
            view_id = request.env.ref('stock_barcode.stock_picking_kanban').id
            return {
                'action': {
                    'name': 'Open List Stock Picking',
                    'res_model': 'stock.picking',
                    'views': [(view_id,'kanban')],
                    'type': 'ir.actions.act_window',
                    'context': {"search_default_available": True, 'search_default_to_do_transfers': True},
